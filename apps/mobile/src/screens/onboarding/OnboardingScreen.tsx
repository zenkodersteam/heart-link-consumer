...
export default function OnboardingScreen() {
  ...
  const params = useRoute<RootRoute<'Onboarding'>>().params ?? {};
    ...
    try {
      if (step.key === 'name') {
        ...
        if (editingSection) {
          ...
          navigation.reset({ index: 0, routes: [{ name: 'Tabs' }, { name: 'EditProfile' }] });
          return;
        }
      ...
      stickyHeader={
        ...
        <View style={styles.progressBlock}>
          <View style={styles.progressMeta}>
            {/* What is left, not only where you are. "Step 4 of 9" is a
                position; "two questions left" is a reason to keep going, and
                the count is what made changing one answer feel like starting
                the sign-up over. */}
            <Text style={styles.progressStep} maxFontSizeMultiplier={1.3}>
              {STEPS.length - stepIndex === 1
                ? 'Last question'
                : `${STEPS.length - stepIndex - 1} more ${STEPS.length - stepIndex - 1 === 1 ? 'question' : 'questions'}`}
            </Text>
    ...
    >
      ...
      {editingSection ? (
        <Button
          ...
          variant="ghost"
          disabled={saving}
          onPress={() => navigation.reset({ index: 0, routes: [{ name: 'Tabs' }, { name: 'EditProfile' }] })}
        />
      ) : null}
